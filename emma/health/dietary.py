...
prompt = Template(
    """我的体重是59.3公斤，身高1.77，孕周是7周，血糖正常，口味是 {{ cuisine }}。
    请给我推荐一个一天的食谱。要求：
    1. 符合孕妇营养健康要求，每天分为5餐：早餐、早加餐（一般是水果、坚果、乳制品等）、中餐、下午茶（一般是水果、坚果等，注意不要和早加餐冲突）、晚餐。
    2. 要求推荐具体食物和食物量（按照克计算） 
    3. 对于早、中、晚三餐，根据推荐的食物，组合成一个或多个菜谱，给出菜的名称（title）、菜的做法（description）、包含的食物和用量（[{"name": "鱼", "amount": "200"}]。
    返回如下JSON格式：
    {"dailyfood": [
        {"name": "早餐", "recipes": [
    ...
    请给出符合上述要求，但不同于example的食谱，注意输出格式
    """
)
