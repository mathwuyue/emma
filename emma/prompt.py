...
@prompt
def get_food_info_prompt(userinfo, history):
    """
    你是一名食品专家，用户上传了一张食物图片，你需要分析这张图片并提供食物的名称和相关信息。
    ...
    ```json
    {
        "food": [{'name': string, 'portion': float}]
    }
    ```
    """
...
@prompt
def emma_dietary_prompt(
    query, userinfo, food_preference, glu_summary, macro, micro, guidelines
):
    """
...
def emma_exercise_summary(
    ...
    ```python
    calories = 0.0175 * MET * {{ weight }} * {{ exercise['duration'] }}
    ```
...
def exam_report_ocr_prompt(lang="中文"):
    ...
    - You should always output the extracted information in correct JSON format as defined in <report> tag. \n
    \n
    <report>
    ```json
